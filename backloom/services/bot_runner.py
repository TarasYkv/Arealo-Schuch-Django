...
class BotRunner:
    ...
    def _build_agent_task(self) -> str:
        ...
        return (
            ...
            f'⚠️ KRITISCH — TYPISCHE FLOWS (ein oder beide treffen zu):\n\n'
            f'**Flow A: Zweistufiger Account+Eintrag**\n'
            f'1. Account registrieren (Email + Passwort + ggf. Captcha)\n'
            f'2. DANACH separates "Firma eintragen" / "Eintrag erstellen" /'
            f' "Mein Eintrag" / "Profil ausfuellen"-Formular ausfuellen\n\n'
            f'**Flow B: Direkt-Eintrag mit Vorschau-Bestaetigung**\n'
            f'1. Form mit Firmen-Daten ausfuellen\n'
            f'2. ⚠️ ERSTER Submit-Button heisst oft NUR "Vorschau-Eintrag" / '
            f'   "Vorschau" / "Preview" / "Daten pruefen" — das ist KEINE Speicherung!\n'
            f'3. Auf der naechsten Seite/Sektion gibt es einen FINALEN Button:\n'
            f'   "Eintrag jetzt speichern", "Endgueltig absenden", "Bestaetigen", '
            f'   "Final speichern", "Ok, eintragen". Erst NACH diesem Klick ist der '
            f'   Eintrag wirklich gespeichert.\n'
            f'4. Wenn der Submit dich auf die GLEICHE Form-Seite zurueckwirft (URL bleibt '
            f'   /eintragen.php oder aehnlich): das heisst Pflichtfelder fehlen oder '
            f'   sind ungueltig — NICHT als Erfolg werten!\n\n'
            ...
            f'⚠️ PFLICHTFELDER DIE OFT VERGESSEN WERDEN:\n'
            f'- **Nachname** (separat von Vorname — bitte BEIDE fuellen)\n'
            f'- **Branche / Branchenrichtung** (Auswahl-Liste oder Textarea — wenn '
            f'   Pflicht: "Geschenke / Personalisierung / Handwerk")\n'
            f'- **Firmenprofil / Beschreibung / Ueber uns** (Textarea, oft mehrere '
            f'   Zeilen — den BIO-Text aus dem Beschreibungs-Feld unten hier rein)\n'
            f'- **AGB-Checkbox** (sehr oft oben oder unten der Form, MUSS gehakt sein)\n'
            f'- **Datenschutz-Checkbox** (separat von AGB!)\n'
            f'- **Newsletter-Checkbox** ist oft OPTIONAL — nicht hakeln wenn nicht muss\n\n'
            f'⚠️ EMAIL-EINDEUTIGKEIT:\n'
            f'Wenn du registrierst und Site sagt "Email bereits vergeben" / '
            f'"Konto existiert" / "Diese Adresse ist schon registriert" — sofort '
            f'done(success=false, text="ALREADY_REGISTERED: ..."). NICHT Email aendern '
            f'oder einloggen versuchen — das gibt es bei uns nicht.\n\n'
