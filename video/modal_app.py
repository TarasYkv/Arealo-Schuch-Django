...
CLS_CFG = dict(image=image, gpu="L40S", volumes={MODEL_DIR: vol}, timeout=3600, scaledown_window=30, max_containers=1)
...
class LTX2Distilled:
    ...
    def generate_video(self, **kw):
        ...
        result = self.pipe(image=img, prompt=full_prompt, negative_prompt=neg,
...
class LTX2Full:
    ...
    def generate_video(self, **kw):
        ...
        result = self.pipe(conditions=conditions or None, prompt=full_prompt,
            negative_prompt=neg, width=width, height=height, num_frames=121,
