    ...
    def generate_pot_image(self, engraving_text: str, scene_description: str,
        ...
        if base_pot_image_path:
            prompt = (
                ...
                f"\n\n### REFERENZBILD-TREUE (was übernommen werden MUSS): ###"
                f"\nForm, Farbe (cremeweiß matt), Material, Proportionen des Topfes — sowie "
                f"Schriftart, Wortlaut, Position der Gravur und Logo — müssen 1:1 vom Referenzbild "
                f"übernommen werden. Der Text '{engraving_text}' muss EXAKT identisch dargestellt "
                f"werden (Schreibschrift: {engraving_style})."
                f"\n\n### ANPASSUNG AN DIE SZENE (was NEU sein muss): ###"
                f"\nLicht und Schatten DÜRFEN UND MÜSSEN sich an die Szene anpassen — übernimm "
                f"NICHT die flache Studio-Beleuchtung des Referenzbildes. Stattdessen:"
                f"\n• Lichtrichtung muss zur Szene passen (Sonneneinfall, Fensterlicht, Innenraum)"
                f"\n• Farbtemperatur muss zur Umgebung passen (warmes Garten-Licht, kühles Innenraum-Licht)"
                f"\n• Topf wirft realistischen Schatten auf die Hand/den Untergrund passend zur Lichtquelle"
                f"\n• Subtile Reflexionen der Umgebung auf der glasierten Topfoberfläche"
                f"\n• Bei Bewegung: leichte Tiefenunschärfe konsistent mit dem Rest der Szene"
                f"\n\n### REALISMUS-CHECK (gegen 'wie hineinkopiert'-Look): ###"
                f"\nDer Topf darf NICHT wie ein Sticker oder Composite wirken. Vermeide:"
                f"\n• Zu scharfe Kanten, wenn Hintergrund unscharf ist"
                f"\n• Zu perfekte Beleuchtung, wenn die Szene natürliches Licht hat"
                f"\n• Fehlende Schatten unter/neben dem Topf"
                f"\n• Beleuchtungsrichtung, die zum Topf nicht zur Szene passt"
                f"\nDer Topf MUSS organisch in die Szene integriert sein — wie wirklich fotografiert, "
                f"nicht wie nachträglich eingefügt."
                f"\n\n### TOPFGRÖSSE (KRITISCH — sowohl ZU KLEIN als auch ZU GROSS sind falsch): ###"
                f"\nDer Topf ist 14 cm hoch und 14 cm Durchmesser — eine handliche, "
                f"mittlere Größe. KEINE winzige Tasse, KEIN großer Pflanzkübel."
                f"\n\nGrößenvergleiche, an die du dich halten MUSST:"
                f"\n• Etwa wie ein Volleyball oder eine kleine Honigmelone"
                f"\n• Eine erwachsene Person hält den Topf MIT BEIDEN HÄNDEN — "
                f"  EINE Hand allein reicht NICHT zum Umschließen"
                f"\n• Etwas größer als eine Müslischale, etwas kleiner als ein menschlicher Kopf"
                f"\n• Topfdurchmesser ungefähr so breit wie der Abstand zwischen "
                f"  zwei nebeneinanderliegenden flachen Händen"
                f"\n\nWenn eine Person im Bild ist:"
                f"\n• Person hält den Topf typischerweise mit beiden Händen"
                f"\n• Topf-Höhe ungefähr Brust-zu-Bauch-Bereich, wenn vor dem Körper gehalten"
                f"\n• Topf etwa 60-70% so hoch wie der Kopf der Person, NICHT winzig"
                f"\n\nVERBOTEN (beides ist FALSCH):"
                f"\n• ZU KLEIN: Topf wie eine Tasse, Trinkglas, Espresso-Schale (das ist 8-10 cm)"
                f"\n• ZU GROSS: Pflanzkübel, Gartenkübel, große Vasen über 25 cm"
                f"\n• Topf, der mit zwei Fingern getragen werden kann"
                f"\nDer Topf wirkt substantiell und greifbar, nicht zerbrechlich-winzig."
                f"\n\n### BILD-TYP (WICHTIGSTE ANWEISUNG — bestimmt Komposition und Stil): ###"
                f"\n{image_type_prompt}"
                f"\n\n### THEMATISCHER KONTEXT (Stimmung/Umgebung): ###"
                f"\n{scene_description}"
                f"\n\n### TECHNISCHE ANFORDERUNGEN: ###"
                f"\nProfessionelles Produktfoto, hohe Qualität, professionelle Beleuchtung, "
                f"warme Atmosphäre. Die Gravur '{engraving_text}' muss klar lesbar sein. "
                f"Quadratisches Format."
            )
        else:
            ...
            prompt = (
                f"Erstelle ein Produktfoto vom Typ '{image_type_label}'. "
                f"\n\n### TOPF-SPEZIFIKATION (KRITISCH — Größe richtig kalibrieren): ###"
                f"\nDer Blumentopf: {pot_desc}. "
                f"\nGröße: 14 cm hoch und 14 cm Durchmesser — handliche mittlere Größe, "
                f"weder winzig wie eine Tasse noch riesig wie ein Pflanzkübel."
                f"\nKonkrete Größenvergleiche:"
                f"\n• Etwa wie ein Volleyball oder eine kleine Honigmelone"
                f"\n• Eine Person hält den Topf MIT BEIDEN HÄNDEN — eine Hand reicht nicht"
                f"\n• Etwas größer als eine Müslischale, etwas kleiner als ein menschlicher Kopf"
                f"\nWenn Person im Bild: Topf wird mit beiden Händen vor dem Körper gehalten, "
                f"Höhe etwa 60-70% des Kopfes der Person."
                f"\nVERBOTEN (beides ist FALSCH): Tasse/Trinkglas/Espressoschale (zu klein) "
                f"ODER Pflanzkübel/Übertopf/Gartenkübel (zu groß)."
                ...
                f"(nicht aufgemalt, nicht aufgedruckt). Der Text muss EXAKT '{engraving_text}' "
                f"lauten und vollständig lesbar sein. Falls ein Logo zur Gravur gehört, "
                f"halte es konsistent — gleiches Motiv in jedem Bild."
                f"\n\n### BILD-TYP (WICHTIGSTE ANWEISUNG — bestimmt Komposition und Stil): ###"
                f"\n{image_type_prompt}"
                f"\n\n### THEMATISCHER KONTEXT (Stimmung/Umgebung): ###"
                f"\n{scene_description}"
