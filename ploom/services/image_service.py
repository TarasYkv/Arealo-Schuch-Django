    ...
    def generate_pot_image(self, engraving_text: str, scene_description: str,
        ...
        if base_pot_image_path:
            prompt = (
                ...
                f"\n\n### TOPFGRÖSSE (kritisch für Realismus): ###"
                f"\nDer Topf ist klein: ca. 14 cm hoch UND 14 cm Durchmesser (also kompakt, fast würfelförmig). "
                f"Er passt bequem in eine Erwachsenen-Hand. Stelle ihn IMMER in dieser realistischen "
                f"Größe dar — nicht zu groß (kein bodenlanger Pflanzkübel), nicht zu klein (kein Mini-Topf). "
                f"Wenn Personen oder andere Objekte im Bild sind, achte auf korrekte Größenverhältnisse "
                f"(Kaffeetasse ~10 cm zum Vergleich)."
            ...
            prompt = (
                f"Erstelle ein Produktfoto vom Typ '{image_type_label}'. "
                f"\n\n### TOPF-SPEZIFIKATION (kritisch): ###"
                f"\nDer Blumentopf muss so aussehen: {pot_desc}. "
                f"Größe: 14 cm hoch UND 14 cm Durchmesser (kompakt, fast würfelförmig, "
