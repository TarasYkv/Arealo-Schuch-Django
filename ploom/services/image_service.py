    ...
    def generate_pot_image(self, engraving_text: str, scene_description: str,
        ...
        if base_pot_image_path:
            prompt = (
                ...
                f"\n\n### TOPFGRÖSSE (KRITISCH — wird oft VIEL ZU GROSS dargestellt): ###"
                f"\nDer Topf ist sehr klein: NUR 14 cm hoch und 14 cm Durchmesser. "
                f"Konkrete Größenvergleiche, an die du dich halten MUSST:"
                f"\n• DEUTLICH kleiner als ein menschlicher Kopf (Kopf ist ~22 cm breit)"
                f"\n• Etwa wie eine Cappuccino-Tasse oder ein normales Trinkglas"
                f"\n• Eine einzelne Erwachsenen-Hand kann den Topf SEITLICH umschließen"
                f"\n• Der Topf passt vollständig zwischen Daumen und Mittelfinger einer Hand"
                f"\n\nWenn eine Person im Bild ist:"
                f"\n• Topf MUSS deutlich kleiner sein als der Kopf der Person"
                f"\n• Topf darf NIEMALS so breit sein wie das Gesicht"
                f"\n• Topf darf nicht beide Hände komplett ausfüllen"
                f"\n\nVERBOTEN (das sind alles FALSCH):"
                f"\n• Pflanzkübel, Gartenkübel, Übertöpfe, Vasen — alles über 18 cm"
                f"\n• Töpfe in der Größe eines menschlichen Kopfes oder größer"
                f"\n• Töpfe, die mit beiden Händen umfasst dargestellt sind, weil sie zu groß sind"
                f"\nWenn der Topf gleich groß oder größer als der Kopf erscheint, "
                f"ist das Bild FALSCH und der Topf MUSS deutlich verkleinert werden."
            ...
            prompt = (
                f"Erstelle ein Produktfoto vom Typ '{image_type_label}'. "
                f"\n\n### TOPF-SPEZIFIKATION (KRITISCH — wird oft VIEL ZU GROSS dargestellt): ###"
                f"\nDer Blumentopf muss so aussehen: {pot_desc}. "
                f"\nGröße: sehr klein, NUR 14 cm hoch und 14 cm Durchmesser."
                f"\nKonkrete Größenvergleiche:"
                f"\n• DEUTLICH kleiner als ein menschlicher Kopf (Kopf ~22 cm)"
                f"\n• Etwa wie eine Cappuccino-Tasse oder ein normales Trinkglas"
                f"\n• Eine einzelne Erwachsenen-Hand kann den Topf SEITLICH umschließen"
                f"\nWenn Person im Bild: Topf MUSS deutlich kleiner sein als der Kopf der Person."
                f"\nVERBOTEN: Pflanzkübel, Übertöpfe, Vasen, Schalen — alles über 18 cm. "
                f"Wenn der Topf gleich groß oder größer als der Kopf erscheint, ist das Bild FALSCH."
