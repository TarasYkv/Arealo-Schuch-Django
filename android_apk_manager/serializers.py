"""
Android APK Manager Serializers
REST API Serialization für Android Apps und Versionen
"""
...
class AppVersionListSerializer(serializers.ModelSerializer):
    """Lightweight serializer für Version-Listen"""
    ...
    class Meta:
        ...
        fields = [
            ...
            'min_android_version', 'uploaded_at'
        ]
...
class AppVersionDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AppVersion
        fields = [
            'id', 'app', 'app_name', 'package_name',
            'version_name', 'version_code', 'release_notes',
            'channel', 'min_android_version',
            'apk_file', 'file_size', 'file_size_mb',
            'download_count', 'download_url',
            'is_active', 'is_current_for_channel',
            'uploaded_at', 'updated_at'
        ]
        read_only_fields = ['id', 'file_size', 'download_count', 'uploaded_at', 'updated_at']
    ...
    def get_download_url(self, obj):
        ...
        if request:
            ...
            return request.build_absolute_uri(
                reverse('android_apk_manager:download-apk', kwargs={'version_id': str(obj.id)})
            )
        return None


class AndroidAppListSerializer(serializers.ModelSerializer):
    """Lightweight serializer für App-Listen"""
    ...
    class Meta:
        model = AndroidApp
        fields = [
            'id', 'name', 'package_name', 'description', 'icon',
...
class AndroidAppDetailSerializer(serializers.ModelSerializer):
...
class DownloadLogSerializer(serializers.ModelSerializer):
    """Serializer für Download-Statistiken"""
...
class UpdateCheckSerializer(serializers.Serializer):
    """Serializer für Update-Check Request von Android-App"""
...
class UpdateCheckResponseSerializer(serializers.Serializer):
    """Serializer für Update-Check Response"""
